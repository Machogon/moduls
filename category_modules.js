...
// =========================================================================
// ====================== Новая категория: Автолампы ======================
// =========================================================================
    "Автолампы": (() => {
        // Списки значений для "Тип лампы" и "Цоколь" (пока с примерами, ты добавишь свои позже)
        const lampTypesOriginal = ["W5W", "H6W", "H11", "P21/5W", "PY21W"];
        const socketTypesOriginal = ["W 2,1 x 9,5d", "BAX 9s", "PGJ19-2", "BAY 15d", "BAU 15s"];

        // Нормализованные списки для поиска (отсортированы по длине)
        const lampTypes = lampTypesOriginal
            .map(normalizeString)
            .sort((a, b) => b.length - a.length);
        const socketTypes = socketTypesOriginal
            .map(normalizeString)
            .sort((a, b) => b.length - a.length);
        ...
        return createCategoryModule(
            "Автолампы",
            {
                // Тип лампы (ID 27127) - массив значений, без ключевых слов
                "27127": lampTypesOriginal,
                // Цоколь (ID 27128) - массив значений, без ключевых слов
                "27128": socketTypesOriginal,
                // Назначение (ID 27126)
                "27126": {
                ...
                // Цвет света (ID 253287)
                "253287": {
                    "Белый": ["белый", "white"],
                    "Желтый": ["желтый", "yellow"],
                    "Красный": ["красный", "red"],
                    "Зеленый": ["зеленый", "green"],
                    "Синий": ["синий", "blue"],
                    "Фиолетовый": ["фиолетовый", "purple"],
                    "Оранжевый": ["оранжевый", "amber"],
                    "Теплый синий": ["теплый синий", "warm blue"],
                    "Холодный синий": ["холодный синий", "cold blue"]
                },
                // Количество предметов (ID 92802)
                "92802": {
                    "1": ["1"],
                    "2": ["2"],
                    "4": ["4"],
            ...
            {
                // Опции, если нужны (например, stopWords)
            },
            {
                // Кастомный determineValues для "Автоламп"
                determineValues: function(title) {
                    if (!window.isScriptRunning) return null;
                    const lowerTitle = normalizeString(title);
                    const result = {};

                    // Тип лампы (ID 27127)
                    const lampTypeIndex = lampTypes.findIndex(type => new RegExp(`\\b${type}\\b`).test(lowerTitle));
                    if (lampTypeIndex !== -1) {
                        result["27127"] = lampTypesOriginal[lampTypeIndex];
                        console.log(`🔍 Найден Тип лампы: ${result["27127"]}`);
                    }

                    // Цоколь (ID 27128)
                    const socketTypeIndex = socketTypes.findIndex(socket => new RegExp(`\\b${socket}\\b`).test(lowerTitle));
                    if (socketTypeIndex !== -1) {
                        result["27128"] = socketTypesOriginal[socketTypeIndex];
                        console.log(`🔍 Найден Цоколь: ${result["27128"]}`);
                    }

                    // Остальные параметры через ключевые слова
                    for (const [paramId, mappings] of Object.entries(this.parameters)) {
                        if (paramId === "27127" || paramId === "27128") continue; // Пропускаем Тип и Цоколь
                        for (const [value, keywords] of Object.entries(mappings)) {
                            if (keywords.some(kw => lowerTitle.includes(kw.toLowerCase()))) {
                                result[paramId] = value;
                                console.log(`🔍 Найден параметр ${paramId}: ${value}`);
                                break;
                            }
                        }
                    }
                    ...
                    if (!result["92802"]) {
                        result["92802"] = "1";
                        console.log(`🔍 Установлено Количество предметов по умолчанию: 1`);
                    }
